...
export default function FeesStudentList() {
  ...
  // ⭐ ADDED — Search ke liye state
  const [search, setSearch] = useState("");

  // ⭐ ADDED — Filtered students list
  const filteredStudents = students.filter((s) =>
    s.name.toLowerCase().includes(search.toLowerCase())
  );

  const SCHOOL_API_BASE =
    (window as any).__SCHOOL_API_BASE__ ||
    (window as any).REACT_APP_SCHOOL_API_BASE ||
    "https://school-bos-backend.onrender.com/schoolApp";
  ...
  useEffect(() => {
    ...
    fetch(`${SCHOOL_API_BASE}/classes/${encodeURIComponent(classId)}/`)
      ...
      .then((c) => {
        if (c)
          setClassLabel(`${c.class_name}${c.section ? ` - ${c.section}` : ""}`);
      })
      .catch(() => {});

    const url = `${SCHOOL_API_BASE}/class/${encodeURIComponent(
      classId
    )}/students/`;
        ...
        (async () => {
          try {
            const updated = await Promise.all(
              mapped.map(async (stu) => {
                try {
                  const resp = await fetch(
                    `${SCHOOL_API_BASE}/fee/?student=${stu.id}`
                  );
                  ...
                  const total = fees.reduce(
                    (sum: number, f: any) =>
                      sum + (Number(f.total_amount) || 0),
                    0
                  );
                  const paid = fees.reduce(
                    (sum: number, f: any) =>
                      sum + (Number(f.paid_amount) || 0),
                    0
                  );
                  const status =
                    paid >= total
                  ...
                  const lastDate = last
                    ? last.due_date || last.dueDate || null
                    : null;
    ...
    try {
      const res = await fetch(`${SCHOOL_API_BASE}/fee/`, {
        ...
        body: JSON.stringify({
          ...
          due_date: dueDate,
          status:
            Number(total) === Number(paid) ? "Paid" : "Pending",
        }),
      });
      ...
      if (res.ok) {
        ...
        const newTotal =
          respJson && respJson.total_amount !== undefined
            ? Number(respJson.total_amount)
            : Number(total);
        const newPaid =
          respJson && respJson.paid_amount !== undefined
            ? Number(respJson.paid_amount)
            : Number(paid);
  ...
  return (
    <main className="p-6 bg-gray-50 min-h-screen relative">
      ...
      {/* ⭐ ADDED — Search bar connected with state */}
      <input
        type="text"
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow">
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            {/* ⭐ ADDED — filteredStudents used here */}
